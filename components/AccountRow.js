...
export default function AccountRow(props) {
    ...
    const TLM_yesterday = async (user) => {
        ...
        let yesterday = `${yes.getUTCFullYear()}-${yes.toISOString().slice(5, 6)}-${yes.getUTCDate()}T17:00:00.000Z`
        let today = `${to.getUTCFullYear()}-${yes.toISOString().slice(5, 6)}-${to.getUTCDate()}T16:59:59.999Z`
